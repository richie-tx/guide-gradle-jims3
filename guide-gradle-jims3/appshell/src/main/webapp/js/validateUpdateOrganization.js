function validate() {
  ...
  if (frm.PrimaryName.value == "") {
    ...
    errorMsg = "You must enter an organization name.";
  } else
  if (frm.PrimaryName.value.indexOf('"') > -1) {
    errorField = frm.PrimaryName;
    errorMsg = 'The organization name cannot contain a double quote character.';
  } else
  if (frm.ClientNumber && frm.ClientNumber.value.substring(0,1) == "0") {
    errorField = frm.ClientNumber;
    errorMsg = "Leading zeros are not permitted in the core billing number.";
  } else 
  if (frm.ClientNumber && frm.ClientNumber.value != "" && frm.ClientNumber.value.match(/\D/)) {
    errorField = frm.ClientNumber;
