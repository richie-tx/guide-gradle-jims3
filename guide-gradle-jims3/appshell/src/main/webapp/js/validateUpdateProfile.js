function validate() {
  ...
  if (frm.Code.value == "") {
    errorField = frm.Code;
    errorMsg = "You must enter a profile code.";
  } else
  ...
  if (frm.Code.value.indexOf('"') > -1) {
    errorField = frm.Code;
    errorMsg = 'The profile code cannot contain a double quote character.';
  } else
  if (frm.LongName.value == "") {
    errorField = frm.LongName;
    errorMsg = "You must enter a profile long name.";
  } else
  if (frm.LongName.value.indexOf('"') > -1) {
    errorField = frm.LongName;
    errorMsg = 'The profile long name cannot contain a double quote character.';
  } else
  if (frm.ShortName.value == "") {
    errorField = frm.ShortName;
    errorMsg = "You must enter a profile short name.";
  } else 
  if (frm.ShortName.value.indexOf('"') > -1) {
    errorField = frm.ShortName;
    errorMsg = 'The profile short name cannot contain a double quote character.';
  } else
  ...
  if (frm.Type == -1) {
    errorField = frm.Type;
    errorMsg = "You must select a profile use.";
  } else 
