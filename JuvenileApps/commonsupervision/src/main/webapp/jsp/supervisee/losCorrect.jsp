...
<html:base />
<title><bean:message key="title.heading" /> - supervisee/losCorrect.jsp</title>
...
<script type="text/javascript">
	...
	function validateField(theForm) {
		...
		if(theForm.supervisionLevel != null) {
	   	...
	   	if ((theForm.priorEffectiveDate.value.length > 0) || ((theForm.subsequentEffectiveDate.value.length > 0)) ) {
	   	
	   		if ( (theForm.priorEffectiveDate.value.length > 0) && (theForm.subsequentEffectiveDate.value.length == 0) ) {
	   	
	   			var isEffectiveDateGTPriorEffectiveDate = compareDate1GreaterDate2(theForm.effectiveDate.value, theForm.priorEffectiveDate.value);
	   		
	   			if(!isEffectiveDateGTPriorEffectiveDate) {
					alert('Effective Date must be greater then the prior LOS history\'s effective date of: ' + theForm.priorEffectiveDate.value);
    				theForm.effectiveDate.focus();
    				return false;
				}
	   	
	   		} else if ( (theForm.priorEffectiveDate.value.length == 0) && (theForm.subsequentEffectiveDate.value.length > 0) ) {
	   	
	   			var isSubsequentEffectiveDateGTEffectiveDate = compareDate1GreaterDate2(theForm.subsequentEffectiveDate.value, theForm.effectiveDate.value);
	   		
	   			if(!isSubsequentEffectiveDateGTEffectiveDate) {
					alert('Effective Date must be less then the subsequent LOS history\'s effective date of: ' + theForm.subsequentEffectiveDate.value);
    				theForm.effectiveDate.focus();
    				return false;
				}
	   			...
	   			if ( (!isEffectiveDateGTPriorEffectiveDate) || (!isSubsequentEffectiveDateGTEffectiveDate) ) {
	   				alert('Effective Date must between the prior and subsequent LOS history effective date\'s of: ' + theForm.priorEffectiveDate.value + " - " + theForm.subsequentEffectiveDate.value);
    				theForm.effectiveDate.focus();
    				return false;
	   			}
...
<html:form action="/displayCorrectLOSSummary" target="content" focus="effectiveDate">

<html:hidden name="superviseeForm" property="subsequentEffectiveDate"/>
<html:hidden name="superviseeForm" property="priorEffectiveDate"/>
<html:hidden name="superviseeForm" property="supervisionLevelHistoryId"/>
<div align="center">
									...
									<td valign=top align=center>
										...
										<div class=header><bean:message key="title.CSCD" />&nbsp;-
											<logic:equal name="superviseeForm" property="action" value="<%=UIConstants.UPDATE%>">
												...
												<input type="hidden" name="helpFile" value="commonsupervision/CSCD_Supervisee_Search/Supervisee_Search.htm#|7">
											</logic:equal>
											<logic:equal name="superviseeForm" property="action" value="<%=UIConstants.CREATE%>">
												<bean:message key="prompt.add"/>
												<input type="hidden" name="helpFile" value="commonsupervision/CSCD_Supervisee_Search/Supervisee_Search.htm#|5">
											</logic:equal>
