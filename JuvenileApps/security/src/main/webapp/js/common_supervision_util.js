...
function showBtnGrp(btnGroupName, breakerSymbol,isHistorical,orderChainNum){
	var actualElement;
...
//	btnUniqueName=splitStrArr[1];
	renderButtonsInGrp(btnGroupName,isHistorical,orderChainNum);
...
function verifyLOSAndTransfer(clearField){
	...
	show("passwordContainer", 1)							
	
		if (pw==pwHidden){				
			show("losLinks", 1)
			show("transfersLinks", 1)
			show("passwordContainer", 0)	
...
//checks that tinyMCE text areas do not just contain <br>'s
function validateForBROnly(el, errorMsg){																								
	var re = new RegExp("(<br(\s*/?)>[\w]+)|(^((?!<br(\s*/?)>).)*$)");
	if (el == ""){
		alert(errorMsg);
		return false;
	}else {		
	if (!/(<br(\s*\/?)>[\w]+)|^(((?!<br(\s*\/?)>).)*$)/.test(el)){		
		alert(errorMsg);
		return false;
	}
