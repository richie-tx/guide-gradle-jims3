...
public class HandleJuvenileCasefileOffenseSearchAction extends JIMSBaseAction
{
	 /* (non-Javadoc)
    ...
    */
   public HandleJuvenileCasefileOffenseSearchAction() 
   {
   ...
   public ActionForward returnSelect(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest, HttpServletResponse aResponse) 
   {
	   ...
	   for (int x=0; x<wrkList.size(); x++)
	   {
		   ...
		   if (selVal.equals(jcoEvent.getAlphaCode() ) ) {
			   form.setOffenseId(jcoEvent.getAlphaCode());
			   form.setOffenseDesc(jcoEvent.getShortDescription());
			   break;
		   }
	   }
	   form.setSelectedValue(UIConstants.EMPTY_STRING);
	   ...
	   String forwardStr = UIConstants.SUCCESS + form.getFromPage();
