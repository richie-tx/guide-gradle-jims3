...
public class CompleteSCSInventoryAction extends JIMSBaseAction {	

	/**
	 * @roseuid 464368F103D5
	 */
	public CompleteSCSInventoryAction() {

	}

	protected void addButtonMapping(Map keyMap) {
		keyMap.put("button.link", "completeSCSInventory");		 
	}

	public ActionForward completeSCSInventory(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
			HttpServletResponse aResponse) throws GeneralFeedbackMessageException {	
		
		AssessmentForm assessmentForm = (AssessmentForm)aForm;
		SCSAssessmentForm scsAssessmentForm = (SCSAssessmentForm)this.getSessionForm(aMapping, aRequest, UIConstants.CSC_ASSESS_SCS_ASSESSMENT_FORM, true);
...
//		clear the assessment and scsAssessment form
		...
		scsAssessmentForm.setTaskId(taskId);
		scsAssessmentForm.setAction(UIConstants.UPDATE); 
	    scsAssessmentForm.setSecondaryAction("");
	    scsAssessmentForm.setDefendantId(spnStr);
	    scsAssessmentForm.setSupervisionBeginDate(assessmentForm.getSupervisionBeginDate());
	    scsAssessmentForm.setSupervisionEndDate(assessmentForm.getSupervisionEndDate());
	    scsAssessmentForm.setSupervisionPeriod(CSCAssessmentConstants.ASSESSMENT_SUPERVISION_PRD_ACTV);
	    scsAssessmentForm.setAssessmentTypeId(PDCodeTableConstants.CSC_ASSESSMENTS_TYPE_ID_SCS);
...
//					populate the AssessmentForm with AssessmentSummaryResponseEvent 
				    ...
				    scsAssessmentForm.setSelectedAssessmentBeanId(selectedAssessBeanId.toString());
					break;
				}
			}
		}		
