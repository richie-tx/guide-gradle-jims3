...
public class SubmitVRLawViolationCreateUpdateAction extends JIMSBaseAction {

	/**
	 *@roseuid 47DA9D4602A6
	 */
	public SubmitVRLawViolationCreateUpdateAction() {
...
//    	 TO UNIQUELY IDENTIFY MANUALLY ADDED VALUES FOR REMOVAL UPDATE PAGE AND LEGACY DATA FOR PROCESSING
		...
		for (int g = 0; g < listSize; g++){
			NCLawViolationResponseEvent lvre = (NCLawViolationResponseEvent) vrForm.getCreate1ElementsList().get(g);
...
//        List list = MessageUtil.postRequestListFilter(event, NCLawViolationResponseEvent.class);
		...
		if (returnException == null) {
			vrForm.setConfirmationMessage("Law Violations successfully processed.");
			// remove sort once sortResult tag is fixed
			vrForm.setCreate1ElementsList(UIViolationReportHelper.sortLawViolationList(MessageUtil.compositeToList(response, NCLawViolationResponseEvent.class)));
//	this should only be true on initial create of violation report	
		...
		vrForm.setCurrentLawViolationsList(vrForm.getCreate1ElementsList());
		vrForm.setCurrentLawViolationsComments(vrForm.getCreate1Comments());
		vrForm.setCreate1ElementsList(new ArrayList());
		vrForm.setCreate1Comments("");
