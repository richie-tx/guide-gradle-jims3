package ui.juvenilewarrant.action;

import javax.servlet.http.HttpServletRequest;
import javax.servlet.http.HttpServletResponse;

import messaging.juvenilewarrant.ActivateJuvenileArrestWarrantEvent;
import messaging.juvenilewarrant.reply.ActiveWarrantErrorEvent;
import mojo.km.dispatch.EventManager;
import mojo.km.dispatch.IDispatch;
import mojo.km.messaging.EventFactory;
import mojo.km.messaging.Composite.CompositeResponse;
import mojo.km.utilities.MessageUtil;
import naming.JuvenileWarrantControllerServiceNames;
import naming.UIConstants;

import org.apache.struts.action.Action;
import org.apache.struts.action.ActionMessage;
import org.apache.struts.action.ActionErrors;
import org.apache.struts.action.ActionForm;
import org.apache.struts.action.ActionForward;
import org.apache.struts.action.ActionMapping;

import ui.juvenilewarrant.form.JuvenileWarrantForm;
import ui.juvenilewarrant.helper.JuvenileWarrantFormDirector;

/**
 * @author ryoung
 *
 */
public class SubmitWarrantARRActivateAction extends Action
{

	/**
	 * @roseuid 41E7D9D802B6
	 */
	public SubmitWarrantARRActivateAction()
	{

	}

	/**
	 * @param aMapping
	 * @param aForm
	 * @param aRequest
	 * @param aResponse
	 * @return ActionForward
	 * @roseuid 41E7D8E7016F
	 */
	public ActionForward execute(
		ActionMapping aMapping,
		ActionForm aForm,
		HttpServletRequest aRequest,
		HttpServletResponse aResponse)
	{
		String forwardStr = UIConstants.ACTARR_SUCCESS;
		IDispatch dispatch = EventManager.getSharedInstance(EventManager.REQUEST);
		
		JuvenileWarrantForm jwForm = (JuvenileWarrantForm) aForm;
		ActivateJuvenileArrestWarrantEvent requestEvent =
			(ActivateJuvenileArrestWarrantEvent) EventFactory.getInstance(
				JuvenileWarrantControllerServiceNames.ACTIVATEJUVENILEARRESTWARRANT);		

		requestEvent.setWarrantNum(jwForm.getWarrantNum());
		
		CompositeResponse response = MessageUtil.postRequest(requestEvent);

		ActiveWarrantErrorEvent error =
			(ActiveWarrantErrorEvent) MessageUtil.filterComposite(response, ActiveWarrantErrorEvent.class);
			
		if (error != null)
		{
			ActionErrors errors = new ActionErrors();
			errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.juvenilewarrant.active.warrant.found"));
			saveErrors(aRequest, errors);
			forwardStr = UIConstants.SEARCH_FAILURE;
		}
		else		
		{		
			JuvenileWarrantFormDirector director = new JuvenileWarrantFormDirector(jwForm);
			director.setActivatedWarrantProperties(response);
		}

		return aMapping.findForward(forwardStr);
	}

}
