 ...
 *  
 */
public class DisplayWarrantOICUpdateConfirmAction extends LookupDispatchAction
{
            ...
            throws Exception
    {
        ...
        form.setWarrantTypeUI(UIConstants.UPDATE_OIC_SUCCESS);
        ...
        //Bug 134255: MJCW: Update Warrant - Null value error when select school district but no School Name
        if( StringUtils.isNotBlank(form.getSchoolDistrictId() )){
            
            if( StringUtils.isNotBlank(form.getSchoolCodeId() )){
            ...
            HttpServletResponse aResponse) throws Exception
    {
        ...
        boolean update = UIConstants.UPDATE_OIC_SUCCESS.equals(typeUI);

        if (update)
        {
            this.updateWarrant(form);
            form.setWarrantTypeUI(UIConstants.UPDATE_OIC_CONFIRM);
        }
        else
        {
            form.setWarrantTypeUI(UIConstants.UPDATE_OIC_SUCCESS);
        ...
        if (charges != null)
        {
            ...
            while (i.hasNext())
            {
                ...
                if (petitionEvt.getSequenceNum().equals(form.getSelectedCharge()))
                {
                    ...
                    chargeEvent.setChargeDescription(petitionEvt.getOffense());
                    chargeEvent.setDegree(petitionEvt.getDegree());
                    chargeEvent.setLevel(petitionEvt.getLevel());
                    chargesSelected.add(chargeEvent);

                    form.setPetitionNum(petitionEvt.getPetitionNum());
                    form.setChargeSeqNum(petitionEvt.getSequenceNum());
                    form.setChargeCodeId(petitionEvt.getOffenseCodeId());                    
                    form.setChargeDescription(petitionEvt.getOffense());
                    
                    String chargeCourt = this.getChargeCourt(petitionEvt.getCourtId());
                    form.setChargeCourt(chargeCourt);
                }
            }
        }
    ...
    private String getChargeCourt(String aCourtId)
    {
        GetJuvenileCourtsEvent courtEvent = (GetJuvenileCourtsEvent) EventFactory
                .getInstance(CodeTableControllerServiceNames.GETJUVENILECOURTS);
        IDispatch dispatch = EventManager.getSharedInstance(EventManager.REQUEST);
        courtEvent.setCourt(aCourtId);
    ...
    {
        UpdateJuvenileWarrantEvent event = (UpdateJuvenileWarrantEvent) EventFactory
                .getInstance(JuvenileWarrantControllerServiceNames.UPDATEJUVENILEWARRANT);
        this.setEvent(form, event);        
        CompositeResponse replies = MessageUtil.postRequest(event);  
    }
    ...
    private void setEvent(JuvenileWarrantForm form, UpdateJuvenileWarrantEvent event)
    {
        ...
        event.setWarrantAcknowledgeStatus(PDJuvenileWarrantConstants.WARRANT_ACKNOWLEDGE_NOT_PRINTED);
        event.setWarrantSignedStatus(PDJuvenileWarrantConstants.WARRANT_NOT_SIGNED);
        ...
        event.setWarrantOriginatorName(form.getWarrantOriginatorJudge());
    ...
    /**
     * Computes a change set based on the following rules: If set1 does not
     * exist, then set2 is returned. If set2 does not exist, then nothing is
     * returned. Else compute which values in set2 are not found in set1.
     * 
     * @param set1
     * @param set2
     * @return
