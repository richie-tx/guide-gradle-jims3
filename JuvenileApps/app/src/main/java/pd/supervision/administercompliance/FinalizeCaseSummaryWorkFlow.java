...
public class FinalizeCaseSummaryWorkFlow
{
   
   /**
    * @roseuid 47DA96A803DA
    */
   public FinalizeCaseSummaryWorkFlow() 
   {
   ...
   public void execute(UpdateNCResponseStatusEvent reqEvent) 
   {
	    ...
	    if(vr != null){
		    ...
		    if (!reqEvent.getStatusId().equals("CD")) {
   ...
   private String createCSTask( UpdateNCResponseStatusEvent createTaskEvent ){
	 ...
	 //due date - should be next day unless current day is Friday then next day is Monday
		...
		// on task for CLO's in CS, the court usually contains a prefix of CC or CR which needs to be removed in task
		...
		createCSTaskEvent.setScenario( ViolationReportConstants.PAGEFLOW_SCENERIO2 );			
		createCSTaskEvent.setNcResponseId( createTaskEvent.getNcResponseId() );
		createCSTaskEvent.setTastText( createTaskEvent.getText() );
		createCSTaskEvent.setCriminalCaseId( createTaskEvent.getCaseNumber() );
