...
public class SearchCaseloadUtil
{
    ...
    public static List getCaseload(GetCaseloadEvent event){

        List results = new ArrayList();
        if (CaseloadConstants.SEARCH_CASELOAD_BY_OFFICER.equals(event.getWorkflowInd()))
        {
        	results = getCaseloadByOfficer(event);
        }
        else if (CaseloadConstants.SEARCH_CASELOAD_BY_NAME.equals(event.getWorkflowInd()))
        {
            results = CaseAssignmentOrder.findAllBySuperviseeName(event);
        }
        else if (CaseloadConstants.SEARCH_CASELOAD_BY_SPN.equals(event.getWorkflowInd()))
        {
        	results = getCaseloadByDefendantId(event);
        }
        else if (CaseloadConstants.SEARCH_CASELOAD_BY_QUADRANT.equals(event.getWorkflowInd()))
        {
        	results = getCaseloadByQuadrantId(event);
        }
        else if (CaseloadConstants.SEARCH_CASELOAD_BY_ZIPCODE.equals(event.getWorkflowInd()))
        {
        	results = getCaseloadByZipCode(event);
        }
        return results;
    }
    
    protected static List getCaseloadByOfficer(GetCaseloadEvent event){
        List results = new ArrayList();
    ...
    protected static List getCaseloadByDefendantId(GetCaseloadEvent event){
        List results = new ArrayList();
    ...
    protected static List getCaseloadByQuadrantId(GetCaseloadEvent event){
        List results = new ArrayList();
    ...
    protected static List getCaseloadByZipCode(GetCaseloadEvent event){
        List results = new ArrayList();
     ...
     * @return
     */
    protected static List sendBySupervisee(List results, boolean postResponse)
    {
        ...
        Iterator iterCSEvent = CSEventsReport.findAll(gcEvent);
        while(iterCSEvent.hasNext()){
        	CSEventsReport csEvent = (CSEventsReport) iterCSEvent.next();
        	
        	List list = (List) csEventMap.get(csEvent.getSuperviseeId());
        	if (list != null) 
        	{
        		list.add(csEvent);
        		csEventMap.put(csEvent.getSuperviseeId(), list);
        	} 
        	else
        	{
        		list = new ArrayList();
     ...
     * @return
     */
    private static CaseAssignmentResponseEvent createResponse(ICaseAssignment assignmentBean, Supervisee supervisee, CaseAssignmentOrder caOder, Map csEventMap)
    {
	...
	private static String getMinOfDaysLeft(List cases) {
		long minValue = 0;	
	...
	private static void setSupervisionDates(CaseAssignmentResponseEvent response, Map csEventMap) {
		...
		if(list != null && !list.isEmpty()){
			...
			Collections.sort(list, CSEventsReport.CSEventsComparator);
			Collections.reverse(list);
			
			Iterator results = list.iterator();
			for (;results.hasNext();) {
				CSEventsReport defendantEvent = (CSEventsReport) results.next();			
				String eventTypeId = defendantEvent.getEventTypeId();
				String contactMethod = defendantEvent.getContactMethod();
				String outcome = defendantEvent.getOutcomeCd();
				...
				if (eventTypeId.equals(PDCodeTableConstants.CS_OFFICE_VISIT_TYPE)
					...
					if (PDCodeTableConstants.OTHER_AND_FV_OUTCOME_COMPLETE.equals(outcome) ) {
						if (StringUtils.isNotEmpty(contactMethod) && contactMethod.equals("DI") ){
							lastFaceToFaceInMillis = Math.max(lastFaceToFaceInMillis, eventDateInMillis);										
							calendar.setTimeInMillis(lastFaceToFaceInMillis);	
							lastFaceToFaceDate = calendar.getTime();
						}
