...
public class GetJJSReferralCommand implements ICommand {
	

	/**
	 * @roseuid 4328435B0083
	 */
	public GetJJSReferralCommand()
	{
	...
	{
		GetJJSReferralEvent refEvent = (GetJJSReferralEvent) event;
		...
		if (referralList.hasNext())
		{
			...
			if (ref.getCourtResultId() != null) {
				...
				if(courtDisposition != null) {
					...
					dispCodeRespEvt.setCodeNum(courtDisposition.getCodeNum());
			...
			//List<JJSPetition> petitions = InterviewHelper.getPetitionsForReferral( juvenile, ref );//commented as it blows if no jcjuvenile record
