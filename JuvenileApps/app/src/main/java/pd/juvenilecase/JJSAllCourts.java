 ...
 * @author anpillai
 */
public class JJSAllCourts extends PersistentObject
{
    /**
	 * 
	 */
    ...
    private String courtOID;
    private String detentionOID;
    private String ancillaryOID;
     ...
     * @param event
     */
    static public Iterator<JJSAllCourts> findAll(IEvent event)
    {
	IHome home = new Home();
	...
	if (grantedFeature)
	    return home.findAll(event, JJSAllCourts.class);
	else
	    return filterSealedPurged(home.findAll(event, JJSAllCourts.class));
    }
     ...
     * @return Iterator
     */
    public static Iterator<JJSAllCourts> findAll(String attributeName, String attributeValue)
    {
	...
	boolean grantedFeature = securityManager.isAllowed("JCW-SEALPURGE-VIEW");
	if (grantedFeature)
	    return home.findAll(attributeName, attributeValue, JJSAllCourts.class);
	else
	    return filterSealedPurged(home.findAll(attributeName, attributeValue, JJSAllCourts.class));
    }

    private static Iterator filterSealedPurged(Iterator iter)
    {
	ArrayList<JJSAllCourts> courtDocs = new ArrayList<JJSAllCourts>();
	while (iter.hasNext())
	{
	    //my view contains both court records and detention
	    JJSAllCourts det = (JJSAllCourts) iter.next();
	    if ( det != null && ("COURT".equalsIgnoreCase(det.getRectype()) ||"DETENTION".equalsIgnoreCase(det.getRectype())||"ANCILLARY".equalsIgnoreCase(det.getRectype()))){
		courtDocs.add(det);
	    }
	}
  ...
  /*  public void setCourtDate(Integer courtDate)
     ...
     * @return
     */
    static public JJSAllCourts find(String jjsCOURT_ID)
    {

	JJSAllCourts court = (JJSAllCourts) new Home().find(jjsCOURT_ID, JJSAllCourts.class);
	...
	//resp.setDocketEventId(this.getOID());//THIS NEEDS TO BE DIFFRENT BASED ON RECTYPE
	resp.setRecType(this.getRectype());
	if(this.getRectype().equalsIgnoreCase("COURT")||this.getRectype().equalsIgnoreCase("I.COURT")||this.getRectype().equalsIgnoreCase("S.COURT"))
	{
	    resp.setDocketType("COURT");
	    resp.setDocketEventId(this.getCourtOID());
	}
	else if(this.getRectype().equalsIgnoreCase("DETENTION")||this.getRectype().equalsIgnoreCase("I.DETENTION")||this.getRectype().equalsIgnoreCase("S.DETENTION"))
	{
	    resp.setDocketType("DETENTION");
	    resp.setDocketEventId(this.getDetentionOID());
	}
	else if(this.getRectype().equalsIgnoreCase("ANCILLARY")||this.getRectype().equalsIgnoreCase("I.ANCILLARY")||this.getRectype().equalsIgnoreCase("S.ANCILLARY"))
	{
	    resp.setDocketType("ANCILLARY");
	    resp.setDocketEventId(this.getAncillaryOID());
	}
	...
	if(this.getRectype().equalsIgnoreCase("ANCILLARY"))
	    resp.setHearingType(this.getSettingReason());
	else
	...
	/*if (this.resetDate != null) add in command
	{
	...
	resp.setDetentionId(this.detentionId);
	return resp;
    }
  ...
  //task 147422
    ...
    public String getAncillaryOID()
    {
        return ancillaryOID;
    }

    public void setAncillaryOID(String ancillaryOID)
    {
        this.ancillaryOID = ancillaryOID;
    }

    public String getDetentionOID()
    {
        return detentionOID;
    }

    public void setDetentionOID(String detentionOID)
    {
        this.detentionOID = detentionOID;
    }

    
    public String getCourtOID()
    {
        return courtOID;
    }

    public void setCourtOID(String courtOID)
    {
