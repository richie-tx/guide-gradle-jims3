 ...
 * Window - Preferences - Java - Code Style - Code Templates
 */
public class GenerateNewCasePlanVersionCommand implements ICommand {
	...
	public void execute(IEvent event) throws Exception {
		...
		GenerateNewCasePlanVersionEvent newEvent=(GenerateNewCasePlanVersionEvent)event;
   		String casefileID = newEvent.getCasefileId();
   		JuvenileCasefile casefile = JuvenileCasefile.find(casefileID);
   		String oldCaseplanID = newEvent.getExistingCaseplanId();
   		CasePlan oldCasePlan=CasePlan.find(oldCaseplanID);
   		CasePlan newCasePlan= new CasePlan();
   			newCasePlan.setStatusId(CasePlanConstants.CP_STATUS_PENDING);
		...
		home.bind(newCasePlan);
   		...
   		if(myOldGoals!=null && myOldGoals.size()>0){
   			...
   			while(myOldGoalIter.hasNext()){	
   				...
   				newGoal.setStatusId(oldGoal.getStatusId());	
   				newGoal.setDomainTypeId(oldGoal.getDomainTypeId());
   				newGoal.setEntryDate(oldGoal.getEntryDate());
   				...
   				newGoal.setGoalIntervention(oldGoal.getGoalIntervention()); //added by ER JIMS200075816 MJCW
   				newGoal.setEndRecommendations(oldGoal.getEndRecommendations());
   				newGoal.setEndRecommendationDate(oldGoal.getEndRecommendationDate());
   				newGoal.setCasePlanId(newCaseplanId);
   				newGoal.setClosedDate(oldGoal.getClosedDate());
   				//newGoal.setStatusChangeDate(oldGoal.getStatusChangeDate()); //added new status change date 
   				Home home1=new Home();
   				home1.bind(newGoal);
		...
		helper.createActivity(activityEvt);
   		...
   		if(oldPlacement != null) {
   			...
   			newPlacement.setEntryDate(oldPlacement.getEntryDate());
   			newPlacement.setSupervisionNumber(casefileID);
