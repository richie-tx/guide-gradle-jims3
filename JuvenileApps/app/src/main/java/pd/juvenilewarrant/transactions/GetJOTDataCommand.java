 ...
 *  
 */
public class GetJOTDataCommand implements ICommand
{

    /**
     * @roseuid 416C38340352
     */
    public GetJOTDataCommand()
    {
    ...
    {
        GetJOTDataEvent jotEvent = (GetJOTDataEvent) event;
        IDispatch dispatch = EventManager.getSharedInstance(EventManager.REPLY);
        JuvenileOffenderTracking jot = JuvenileOffenderTracking.find(jotEvent.getDaLogNum());
        if (jot != null)
        {
            // See if there is an active warrant for this Juvenile (Petition# or
            // Juv# and Ref#)
            ...
            // caught or thrown.
            String juvNum = jot.getJuvenileNum();
            //boolean activeWarrantExists =
            // PDJuvenileWarrantHelper.checkForActiveWarrant(juvNum);
            ...
            if (jot.getJuvenileNum() != null && jot.getJuvenileNum().equals("") == false)
            {
                juvenileNum = new Integer(jot.getJuvenileNum());
            }
            boolean activeWarrantExists = PDJuvenileWarrantHelper.findExistingWarrant(juvenileNum, jot.getDaLogNum(), jot
                    .getDateOfBirth(), jot.getFirstName(), jot.getLastName(), null, null);

            if (activeWarrantExists)
            {
                ActiveWarrantErrorEvent error = new ActiveWarrantErrorEvent();
                error.setMessage("There already exists an active Juvenile Warrant for this DA Log Number: " + jot.getDaLogNum());
            ...
            else
            {
                ...
                //PDJuvenileWarrantHelper.sendAssociateValues(dispatch, jot);
