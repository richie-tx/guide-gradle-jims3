 ...
 */
public class JIMS2Account extends PersistentObject implements IAddressable {
	...
	/**
	 * Clears all pd.security.JIMS2AccountType from class relationship
	 * collection.
	 */
	...
	private void initDepartment() {
		if (department == null) {
			department = Department.find(departmentId);//87191(pd.contact.agency.Department) new mojo.km.persistence.Reference(departmentId,
	...
	private void initJIMS2AccountType() {
		if (JIMS2AccountType == null) {
			...
			JIMS2AccountType = new mojo.km.persistence.ArrayList(JIMS2AccountType.class, "JIMS2AccountId",
					"" + getOID());
		}
	}
	...
	private void initPasswordQuestion() {
		if (passwordQuestion == null) {
			passwordQuestion = (Code) new mojo.km.persistence.Reference(passwordQuestionId,
					Code.class, "PASSWORD_QUESTION").getObject();
		}
	}
	...
	/**
	 * Removes a pd.security.JIMS2AccountType from class relationship
	 * collection.
	 */
	...
	public void setDepartment(Department department) {
		...
		this.department = department;//87191 (pd.contact.agency.Department) new mojo.km.persistence.Reference(department).getObject();
	}
	 ...
	 * @param string
	 */
	static public JIMS2Account find(String JIMS2AccountId) {
		JIMS2Account jims2Account = null;
		IHome home = new Home();
		jims2Account = (JIMS2Account) home.find(JIMS2AccountId, JIMS2Account.class);
		return jims2Account;
	}
	...
	static public Iterator findAll() {
		IHome home = new Home();
		Iterator iter = home.findAll(JIMS2Account.class);
		return iter;
	}
	...
	static public Iterator findAll(IEvent event) {
		IHome home = new Home();
		Iterator iter = home.findAll(event, JIMS2Account.class);
		return iter;
	}
	...
	static public Iterator findAll(String attrName, String attrValue) {
		IHome home = new Home();
		return (Iterator) home.findAll(attrName, attrValue, JIMS2Account.class);
	}
	 ...
	 * @param string
	 */
	static public JIMS2Account findByLogonId(String JIMSLogonId) {
		JIMS2Account jims2Account = null;
		IHome home = new Home();
		Iterator iter = (Iterator) home.findAll("JIMS2LogonId", JIMSLogonId, JIMS2Account.class);
		while (iter.hasNext()) {
			jims2Account = (JIMS2Account) iter.next();
			if(jims2Account.getStatus().equals("A"))
			{
			    return jims2Account;
			}
		}
	...
	/**
	 * @return Returns the activatedDate.
	 */
	...
	/**
	 * set the inactivation parameters
	 */
	public void setInactivation() {
		this.setStatus("I");
		...
		this.setInactivatedBy(userName.toString());
		this.setInactiveDate(new Date());
	}
}
