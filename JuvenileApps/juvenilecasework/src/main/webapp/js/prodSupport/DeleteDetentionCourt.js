		...
		.ready(
				function() {
					...
					var deletevalidate = function() 
					{
						...
						if( (newJuvNum == oldJuvNum) && (oldAttorneyConnection == newattorneyConnection ) 
								&& ( newhrgDate == oldhrgDate ) && ( newcrtResult == oldCourtResult )&& 
								(oldBarNum == newBarNum)&&(oldattorneyName == newattorneyName)&& 
								( oldHearingType == newhearingType )&& ( newbookingRefNum == oldReferralNum ))
						{								
...
//					});
					...
					$("#deleteBtn").on("click",	function(){
						...
						var isOk = window.confirm('Are you sure you want to delete the Detention Court Calendar Record?');
...
//												'/JuvenileCasework/DeleteAncillaryCourtCalendarRecord.do');
							...
							$('form')
							.attr('action',
									'/JuvenileCasework/MainMenu.do?selectedMenuItem=deleteDetentionCourtCalendar');
							$('form').submit();
							...
							.on(
									...
									function(e) {
										...
										$('form')
												.attr(
														'action',
														"/JuvenileCasework/PerformDeleteDetentionCourtRecord.do?submitAction=Search Attorney");
										spinner();
										$('form').submit();
									});
					$("#validateBarNumBtn")
							.on(
									...
									function(e) {										
										$('form')
												.attr(
														'action',
														"/JuvenileCasework/PerformDeleteDetentionCourtRecord.do?submitAction=Validate Bar Number");
										spinner();
										$('form').submit();
									});
									...
									function(e) {
										...
										if (val == "") {
											...
											$('form')
													.attr(
															'action',
															"/JuvenileCasework/PerformDeleteDetentionCourtRecord.do?submitAction=Validate Offense");
											$('form').submit();
										}
									});*/
					$("#selectRefBtn")
									...
									function() {
										...
										if (docketEventId != "") {
											...
											$('form')
													.attr(
															'action',
															'/JuvenileCasework/DeleteDetentionCourtRecordQuery.do?&docketId='+ docketEventId);
											spinner();
											$('form').submit();
											return true;
										}
					...
					$("#backToQryBtn")
							.on(
									...
									function() {
										...
										$('form')
												.attr('action',
														'/JuvenileCasework/DeleteDetentionCourtRecordQuery.do?clr=Y');
										$('form').submit();
										return true;
