...
<title><bean:message key="title.heading" />&nbsp;<bean:message
		key="title.juvenileCasework" />&nbsp;- attorneyNameSearch.jsp</title>
...
{
	document.getElementById("selectBtn").disabled = false;
	/* alert(document.getElementsByName("barNum").Value); */
}
function checkInputs()
{
	...
	url = webApp + "PerformUpdateDistrictCourtCalendarRecord.do?submitAction=Find Attorney&attorneyName="+attorneyName;//&juvenileNum=	
	var CalendarWinStatus = localStorage.getItem("CalendarRecordWin");		
	if(CalendarWinStatus == "open")
	{
...
{	
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateDistrictCourtCalendarRecord.do?submitAction=Refresh";
	var CalendarWinStatus = localStorage.getItem("CalendarRecordWin");		
	if(CalendarWinStatus == "open")
	{
		//window is open just refresh it
...
{	
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateDistrictCourtCalendarRecord.do?submitAction=Back";
	var CalendarWinStatus = localStorage.getItem("CalendarRecordWin");		
	if(CalendarWinStatus == "open")
	{
		//window is open just refresh it
...
function Select()
{	
	var barnum = $(	'input:radio[name=selectedValue]:checked').val();	
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateDistrictCourtCalendarRecord.do?submitAction=Select&barNum="+barnum;//from=updateAncillaryCalendar& removed as its not from ancillary for bug fix 179134
	var CalendarWinStatus = localStorage.getItem("CalendarRecordWin");		
	if(CalendarWinStatus == "open")
	{
		//window is open just refresh it
...
<body topmargin="0" leftmargin="0"
	onload="selectSingleAttorney() onKeyDown=" returncheckEnterKeyAndSubmit(event,true);" >
	<!-- onload="selectSingleAttorney() -->

	<html:form action="/PerformUpdateDistrictCourtCalendarRecord"
		target="content" focus="attorneyName">
		<!-- BEGIN HEADING TABLE -->
								...
								<table border="0" cellspacing='1' cellpadding="2" width="100%">
									...
									<tr>
										...
										<td class="formDe"><html:submit property="submitAction"
												...
												<bean:message key="button.findAttorney" />
												...
												<table border="0" width="100%" cellspacing="1"
													...
													<tr bgcolor="#cccccc">
														...
														<td class="subhead"><bean:message
																key="prompt.barNumber" />
															<jims2:sortResults beanName="ProdSupportForm"
																results="attorneyDataList" primaryPropSort="barNum"
														...
														<td class="subhead"><bean:message
																key="prompt.attorneyName" />
															<jims2:sortResults beanName="ProdSupportForm"
																results="attorneyDataList" primaryPropSort="attyName"
																primarySortType="STRING" defaultSortOrder="ASC"
																sortId="2" />
														</td>
													</tr>
		...
		<table border="0" width="100%">
			<tr>
				<td align="center"><html:submit property="submitAction"
					...
					</html:submit> <html:submit property="submitAction" disabled="true"
						onclick="return Select();" styleId="selectBtn">
						<bean:message key="button.select" />
					</html:submit> <html:submit property="submitAction" onclick="return Back();">
