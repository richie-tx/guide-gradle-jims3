...
<html:base />
<title><bean:message key="title.heading"/>&nbsp;<bean:message key="title.juvenileCasework"/>&nbsp;- juvenileDistrictCourtOffenseSearch.jsp</title>
<!--JQUERY FRAMEWORK-->
...
function checkInputs()
{
	...
	if (msg == "") 
	{
		...
		var fld3 = Trim(document.getElementById("dpsId").value);
		var fld4 = Trim(document.getElementById("catId").value);
		url = webApp + "PerformUpdateReferralOffenseRecord.do?submitAction=Find Offenses&acId="+fld1+"&descId="+fld2+"&dpsId="+fld3+"&catId="+fld4;	
		//alert(url);
		var WinStatus = localStorage.getItem("RecordWin");	
		//alert(WinStatus);
		if(WinStatus == "open")
		{
			//window is open just refresh it
			
			 var attorneyWindow = window.open(url, "pictureWindow","height=500,width=800,toolbar=no,scrollbars=yes,resizable=yes,status=yes,dependent=no"); 
			 attorneyWindow.focus();
...
{	
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateReferralOffenseRecord.do?submitAction=Refresh";
	var WinStatus = localStorage.getItem("RecordWin");		
	if(WinStatus == "open")
	{
		//window is open just refresh it
...
{	
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateReferralOffenseRecord.do?submitAction=Back";
	var WinStatus = localStorage.getItem("RecordWin");		
	if(WinStatus == "open")
	{
		 var Window = window.open(url, "pictureWindow","height=500,width=800,toolbar=no,scrollbars=yes,resizable=yes,status=yes,dependent=no"); 
		 Window.focus();		
		return false;
	}
}
function Select()
{	
	var offcode = $('input:radio[name=selectedValue]:checked').val();
	var webApp = "/<msp:webapp/>";
	url = webApp + "PerformUpdateReferralOffenseRecord.do?submitAction=Select&OffCode="+offcode;
	var WinStatus = localStorage.getItem("RecordWin");		
	if(WinStatus == "open")
	{
		//window is open just refresh it
...
<body>
<html:form action="/PerformUpdateReferralOffenseRecord"  target="content" focus="alphaCodeId">
<!-- BEGIN HEADING TABLE -->
  <div class="page-header">
            <h2>Process Juvenile Court Hearings</h2>
            <h2>Search Offense</h2>
        </div>
<!-- END HEADING TABLE -->
<!-- BEGIN ERROR TABLE  -->
<table width="100%">
	...
	<tr>		  
		<td align="center" class="errorAlert"><bean:write name='ProdSupportForm' property='errMessage' /></td>		  
	</tr>   	  
</table>
<!-- END ERROR TABLE  -->
...
<%-- BEGIN SEARCH INPUT TABLE --%>					
						<table border="0" cellspacing='1' cellpadding="2" width="100%">
							<tr class='crtDetailHead'>
								<td colspan="2" class='crtDetailHead'><bean:message key="prompt.searchFor"/> <bean:message key="prompt.offenses"/></td>
							</tr>
							<tr>
								<td class="formDeLabel"><bean:message key="prompt.alpha"/> <bean:message key="prompt.code"/></td>
								<td class="formDe"><html:text name='ProdSupportForm' property='alphaCodeId' size="6" maxlength="6" styleId="acId"/></td>
							</tr>
							<tr>
								<td class="formDeLabel"  width="1%" nowrap="nowrap"><bean:message key="prompt.short"/> <bean:message key="prompt.description"/></td>
								<td class="formDe"><html:text name='ProdSupportForm' property='shortDesc' size="30" maxlength="30" styleId="descId"/></td>
							</tr>
							<tr>
								<td class="formDeLabel"><bean:message key="prompt.DPS"/> <bean:message key="prompt.code"/></td>
								<td class="formDe"><html:text name='ProdSupportForm' property='dpsCodeId' size="8" maxlength="8" styleId="dpsId"/></td>
							</tr>
							<tr>
								<td class="formDeLabel"><bean:message key="prompt.severityLevel"/> </td>
								<td class="formDe"><html:text name='ProdSupportForm' property="categoryId" size="2" maxlength="2" styleId="catId"/></td>
							</tr>
							<tr>
								<td class="formDeLabel"></td>
								<td class="formDe">
									 <html:submit property="submitAction" onclick="return checkInputs();"><bean:message key="button.findOffenses"/></html:submit>		
									 <html:submit property="submitAction" onclick="return Refresh();"><bean:message key="button.refresh"/></html:submit>		
								</td>
							</tr>
						</table>
						<%-- END SEARCH INPUT TABLE --%>
						<div class="spacer4px"></div>
						<logic:notEmpty name="ProdSupportForm" property="offenseResultList">
<%-- BEGIN SEARCH RESULTS TABLE --%>						
							<table border="0" cellspacing='1' cellpadding="2" width="100%">
								<tr>
									<td align="center" style="padding:2px" colspan="2"><br>
										<bean:size id="collSize" name="ProdSupportForm" property="offenseResultList"/>
...
<%-- BEGIN SEARCH RESULTS LIST TABLE --%>									
										<table border="0" width="100%" cellspacing="1" cellpadding="2">				
											<tr bgcolor="#cccccc">
												<td width="1%"></td>
												<td class="subhead"><bean:message key="prompt.alpha" /> <bean:message key="prompt.code" /><jims2:sortResults beanName="ProdSupportForm" results="offenseResultList" primaryPropSort="alphaCode" primarySortType="STRING" defaultSort="true" defaultSortOrder="ASC" sortId="1" /></td>
												<td class="subhead"><bean:message key="prompt.short" /> <bean:message key="prompt.description" /><jims2:sortResults beanName="ProdSupportForm" results="offenseResultList" primaryPropSort="shortDescription" primarySortType="STRING" defaultSortOrder="ASC" sortId="2" /></td>
												<td class="subhead"><bean:message key="prompt.DPS" /> <bean:message key="prompt.code" /><jims2:sortResults beanName="ProdSupportForm" results="offenseResultList" primaryPropSort="dpsCode" primarySortType="STRING" defaultSortOrder="ASC" sortId="3" /></td>
												<td class="subhead"><bean:message key="prompt.severityLevel" /><jims2:sortResults beanName="ProdSupportForm" results="offenseResultList" primaryPropSort="category" primarySortType="STRING" defaultSortOrder="ASC" sortId="4" /></td>
											</tr>
					                        ...
					                        <bean:define id="paginationResultsPerPage" type="java.lang.String"><bean:message key="pagination.recordsPerPage"/></bean:define>
					                         ...
					                         <%-- End Pagination header stuff --%>
			  								<logic:iterate id="offIndex" name="ProdSupportForm" property="offenseResultList" indexId="index">
...
<table border="0" width="100%">
	<tr> 
		<td align="center">
			<html:submit property="submitAction" onclick="return Back();"><bean:message key="button.back"/></html:submit> 				
			<html:submit property="submitAction" disabled="true" styleId="selectBtn" onclick="return Select();"><bean:message key="button.select" /></html:submit>
			<html:submit property="submitAction" onclick="return Back();"><bean:message key="button.cancel"/></html:submit>
		</td>
	</tr>
</table>
<%-- END BUTTON TABLE --%>
