...
<%------------------MODIFICATIONS ----------------------------%>
<%-- 06/23/2011	CShimek  create jsp - based on calendarEventDetails.jsp --%>
    ...
    <td>
      <ul>
        <logic:notEqual name ="calendarEventListForm" property="calendarType" value="PRE">
          <li>Select <b>Close Window</b> button to close this popup.</li>
			...
			<table width="100%">  
				 <logic:equal name="calendarEventListForm" property="juvenileNum" value="<%=(String)currentJuv%>">	
				   <jims2:isAllowed requiredFeatures='<%=Features.JCW_CFV_CALENDAR_BTNS%>'>
					<tr>
						<td align="center">
					        <input type="submit" name="submitAction" value="<bean:message key='button.requestAttorneyAppt'/>"	onclick="return changeFormActionURL(this.form.name, '/<msp:webapp/>handleAttorneyOptions.do', false) && disableSubmit(this, this.form)">
							<html:submit property="submitAction"><bean:message key="button.idInfo"/></html:submit>
							...
							<input type="submit" name="submitAction" value="<bean:message key='button.viewSocialHistoryData'/>" onclick="return ( changeFormActionURL(this.form.name, '/<msp:webapp/>handleSocialHistoryData.do', false) && disableSubmit(this, this.form))">			
						</td>
					</tr>
					</jims2:isAllowed>
				 </logic:equal>
...
<%-- Defect 49803 - wrap button with logic tags so it only displays
     when in "window pop-up" mode . . . "Defect 4692: Calendar Event 
		 Details pop-up -- add Close Window button" added the button 
		 without enclosing it in "logic" tags
--%>
<logic:equal name ="calendarEventListForm" property="calendarType" value="PRE">
