...
<html:base />
<title><bean:message key="title.heading" /> - commonAppFamilyHistorySummary.jsp</title>
...
<script>
	$(document).ready(function(){
		$("#printCommonBtn").click(function(e){
			e.preventDefault();
			spinner();
			$.ajax({
		        url: '/JuvenileCasework/submitCommonAppExitReport.do?submitAction=Print Common App',
		        type: 'post',
		        data: $('form#commonAppExitReport').serialize(),
		        success: function(data, textStatus , xhr) {
		         	if (200 == xhr.status){
		         		$(".overlay").remove();
		         		setTimeout(function(){alert("Documents can be located in the View History Report tab")}, 1000);
		         	}
		        }
		    });
		})
...
<html:form styleId="commonAppExitReport" action="submitCommonAppExitReport.do" target="content">
                        						...
                        						<tr>
                        							<td valign=top>
														...
														<tiles:insert page="../caseworkCommon/commonAppExitReportTabs.jsp" flush="true">
															<tiles:put name="tabid" value="familyHistory"/>
															<tiles:put name="casefileid" value='<%= request.getParameter(PDJuvenileCaseConstants.CASEFILEID_PARAM) %>' />
														</tiles:insert>	
														<!--tabs end-->
                        							</td>
                              	...
                              	<tr>
                                	<td align="center">
                                  	<logic:notEqual name="commonAppForm" property="action" value="<%=naming.UIConstants.CONFIRM_UPDATE%>">
                                 		...
                                 		<logic:equal name="commonAppForm" property="action" value="<%=naming.UIConstants.CONFIRM_UPDATE%>">
                               				<html:submit styleId="printCommonBtn" property="submitAction"><bean:message key="button.printCommonApp"></bean:message></html:submit> 
