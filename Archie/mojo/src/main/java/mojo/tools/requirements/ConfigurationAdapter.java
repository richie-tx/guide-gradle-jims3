...
public class ConfigurationAdapter
{
	...
	* 
		...
		String cdmPackageName)
	{
		...
		if (cdmPkg.IDispatch != 0)
		{
			...
			for (int loci = 0; loci < reqcount; loci++)
			{
				...
				if (reqTypeValue.equals("Class Data Member"))
				{
	...
	{
		synchronizeMappingConfiguration();
	 ...
	 */
	public static void synchronizeMappingConfiguration()
	{
		...
		_Requirement reqCDM)
	{
		...
		for (int i = 0; i < reqcount2; i++)
		{
			...
			if (dispatchID != 0)
			{
				...
				{
					System.out.println("map= " + req.get_Name());
						...
						if (prop != null)
						{
							...
							for (int loci = 0; loci < reqcount; loci++)
							{
								...
								if (childMapType.equals("Parm"))
								{
									...
									_AttrValue convertWildcard =
										childAttrValues.get_Item(
											(Object) "ConvertWildcard",
											enumAttrValueLookups
												.eAttrValueLookup_Label);
									String lConvertWildcard =
										convertWildcard.get_Text();
									fmProp.setConvertWildcard(lConvertWildcard);
								...
								if (fmProp != null)
								{
									...
									_AttrValue formatString =
										childAttrValues.get_Item(
											(Object) "FormatString",
											enumAttrValueLookups
												.eAttrValueLookup_Label);
									String lFormatString =
										formatString.get_Text();
									fmProp.setFormatString(lFormatString);
							...
							if (fmProp != null)
							{
								...
								_AttrValue formatString =
									childAttrValues.get_Item(
										...
										enumAttrValueLookups
											.eAttrValueLookup_Label);
								String lFormatString = formatString.get_Text();
								fmProp.setFormatString(lFormatString);
