package mojo.struts.taglib.layout.treeview;
...
import mojo.struts.taglib.layout.PanelTag;
import mojo.struts.taglib.layout.util.LayoutUtils;
import mojo.struts.taglib.layout.util.MenuInterface;
...
public class TreeViewTag extends PanelTag implements MenuInterface {
	 ...
	 */
	private static final String TREEVIEW_NUMBER = "mojo.struts.taglib.layout.TreeviewTag.TREEVIEW_NUMBER";
	...
	public class TreeViewIds {
		...
		Hashtable namedTreeview;
		Hashtable anonymousTreeview;
		TreeViewIds() {
...
public int doEndLayoutTag() throws JspException {
	...
	if (menus.size()==0 && name!=null) {
		addItems(LayoutUtils.getMenu(pageContext, name).getMenuComponents());
	}
	...
	// Get the treeview id.
	Integer lc_i = computeTreeviewId();
	...
	doPrintMenu(buffer,(MenuComponent[])menus.toArray(new MenuComponent[menus.size()]), (HttpServletRequest)pageContext.getRequest(), pageContext.getServletContext(), bundle, lc_i.toString(), name, lc_styleClass);
	
	buffer.append("</table>");
	
	// If we are using indirection, generates an hidden frame.	
	if (pageContext.getRequest().getAttribute("mojo.struts.taglib.layout.treeview.TreeViewTag.KEY")==null && name!=null) {
		buffer.append("<iframe id=\"treeFrame\" name=\"treeFrame\" style=\"width:0px; height:0px; border: 0px\" src=\"about:blank\"></iframe>");
		pageContext.getRequest().setAttribute("mojo.struts.taglib.layout.treeview.TreeViewTag.KEY", "");
	}
	...
	public static int doPrintMenu(StringBuffer buffer, MenuComponent[] menus, HttpServletRequest in_request, ServletContext in_servletcontext, String in_bundle, String in_path, String in_name, String in_styleClass) throws JspException {
		...
		for (int i=0;i<menus.length;i++) {
			...
			lc_info.path = in_path.length()==0 ? String.valueOf(i) : in_path + "*" + i;
			...
			lc_info.useIndirection = lc_numberOfMenusPrinted >= lc_numberOfMenusToLoad && in_name!=null && lc_info.isClosed;					
			...
			if (lc_info.hasSubMenus) {
				lc_treeviewRenderer.startSubMenu(buffer,key);	
			}
