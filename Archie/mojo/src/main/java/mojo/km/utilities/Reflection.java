...
{
	private static final String DATE_FMT_SHORT = "yyyy-mm-dd";

	private static final String DATE_FMT_LONG = "E MMM dd HH:mm:ss z yyyy";

	private static WeakHashMap accessorMap = new WeakHashMap();

	private static WeakHashMap accessors = new WeakHashMap();

	private static WeakHashMap mutators = new WeakHashMap();
	...
	private synchronized static void saveAccessor(Method aMethod)
	{
		...
		String lSignature = new StringBuilder(lClassName).append(".get").append(lPropertyName).toString();
	...
	private synchronized static void saveMutator(Method aMethod)
	{
		...
		String lSignature = new StringBuilder(lClassName).append(".set").append(lPropertyName).append("(").append(
	...
	public synchronized static Method getAccessorMethod(Class aClass, String aPropertyName)
	{
		if (aClass == null || aPropertyName == null)
			return null;
		String lClassName = aClass.getName();
		aPropertyName = new StringBuilder(aPropertyName.substring(0, 1).toUpperCase()).append(
				aPropertyName.substring(1)).toString();
		String lMethodSignature = new StringBuilder(lClassName).append(".get").append(aPropertyName).toString();
	...
	public synchronized static Method getMutatorMethod(Class aClass, String aPropertyName, Class aParamType)
	{
		...
		aPropertyName = new StringBuilder(aPropertyName.substring(0, 1).toUpperCase()).append(
				aPropertyName.substring(1)).toString();
		String lMethodSignature = new StringBuilder(lClassName).append(".set").append(aPropertyName).append("(")
				.append(lParamTypeName).append(")").toString();
	...
	public synchronized static List getMutatorMethods(Class aClass, String aPropertyName)
	{
		if (aClass == null || aPropertyName == null)
		{
			return null;
		...
		aPropertyName = new StringBuilder(aPropertyName.substring(0, 1).toUpperCase()).append(
				aPropertyName.substring(1)).toString();
		String lMethodSignature = new StringBuilder(lClassName).append(".set").append(aPropertyName).toString();
		List mutators = (List) methods.get(lMethodSignature);
		if (mutators == null)
		{
			mutators = new Vector();
			for (Iterator i = getMutatorMethods(aClass); i.hasNext();)
			{
				...
				if (lPropertyName != null && lPropertyName.equalsIgnoreCase(aPropertyName))
				{
					mutators.add(lCandidate);
				}
			}
			methods.put(lMethodSignature, mutators);
		}
		return mutators;
	}
	...
	public static void invokeMutatorMethod(Class aClass, Object aBean, String aPropertyName, Object anArgument)
	{
		...
		if (anArgument == null)
		{
			List mutators = getMutatorMethods(aClass, aPropertyName);
			int len = mutators.size();
			for (int i = 0; i < len; i++)
			{
				lMutator = (Method) mutators.get(i);
			}
		}
		else
		{
			...
			if (lMutator == null)
			{
				if (lValue instanceof String)
				{
					List mutators = getMutatorMethods(aClass, aPropertyName);
					int len = mutators.size();
					for (int j = 0; j < len && lMutator == null; j++)
					{
						Method lCandidate = (Method) mutators.get(j);
						Class paramType = lCandidate.getParameterTypes()[0];

						Object lTempValue = convertValue(paramType, (String) lValue);
						if (lTempValue != null)
						{
							lValue = lTempValue;
							lMutator = lCandidate;
	...
	private static Object convertValue(Class aClass, String anValue)
	{
		...
		try
		{
			...
			else if (className.equals("java.util.Date") || className.equals("java.sql.Date"))
			{
				...
				try
				{
					// try longer format based on toString
					d = DateUtil.stringToDate(anValue, DATE_FMT_LONG);
				}
				catch (ParseRuntimeException p)
				{
					// try shorter format based on Date.valueOf
					d = DateUtil.stringToDate(anValue, DATE_FMT_SHORT);
				}
				return d;
			}
		...
		catch (Exception e)
		{
			...
			builder.append("Reflection error trying to convert <");
			builder.append(anValue);
			builder.append("> to a ");
