//Source file: C:\\views\\CommonSupervision\\app\\src\\ui\\supervision\\administercompliance\\administerviolationreport\\action\\SubmitVRFeeHistoryCreateUpdateAction.java
...
public class SubmitVRFeeHistoryCreateUpdateAction extends JIMSBaseAction {

	/**
	 *@roseuid 47DA9D460007
	 */
	public SubmitVRFeeHistoryCreateUpdateAction() {
...
//   	 TO UNIQUELY IDENTIFY MANUALLY ADDED VALUES FOR REMOVAL UPDATE PAGE AND LEGACY DATA FOR PROCESSING
		...
		for (int g = 0; g < listSize; g++){
			NCFeeResponseEvent ncre = (NCFeeResponseEvent) vrForm.getCreate1ElementsList().get(g);
		...
		if (returnException == null) {
			vrForm.setConfirmationMessage("Fee History successfully processed.");
			// remove sort once sortResult tag is fixed
			vrForm.setCreate1ElementsList(UIViolationReportHelper.sortFeeHistoryList(MessageUtil.compositeToList(response, NCFeeResponseEvent.class)));
//	this should only be true on initial create of violation report	
		...
		vrForm.setCurrentFeeHistoryList(vrForm.getCreate1ElementsList());
		vrForm.setCurrentFeeHistoryComments(vrForm.getCreate1Comments());
		vrForm.setCreate1ElementsList(new ArrayList());
		vrForm.setCreate1Comments("");
