...
public class AddCasenoteForOfficerAssignmentAction extends JIMSBaseAction {
	...
	public ActionForward displayCaseNote(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
		...
		String crimCaseId = caseAssignmentForm.getCaseNum();
		...
		if ( "Y".equals( currentStateCd) || "W".equals( currentStateCd ) ){
			...
			// Find out if there is a program unit change
			// Find out if program unit has state reporting
			...
			for ( int x =0 ; x < referralList.size() ; x++ ){
				...
				if ( !"E".equals( refResponse.getReferralStatusCode()) && refResponse.isProgramUnitRef() ){
						...
						if ( currentStateCd == null && refResponse.getCriminalCaseId().equals( crimCaseId )){
		...
		String caseNoteDate = caseAssignmentForm.getOfficerAssignmentDate();
		caseAssignmentForm.setCasenoteDate(caseNoteDate);
		StringBuffer sb = new StringBuffer();
		sb.append( "Paper Casefile received for " );
		...
		sb.append(" on ");
		sb.append(DateUtil.getCurrentDateString(DateUtil.DATE_FMT_1));//Acknowledgment Date
		sb.append(" and has been assigned to ");
		sb.append(caseAssignmentForm.getOfficerName().getFormattedName());
		sb.append(" on ");
		sb.append(caseNoteDate); //Position assignment date.
		sb.append(".");
