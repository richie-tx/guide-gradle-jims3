...
public class CompleteForceFieldAction extends JIMSBaseAction {	

	/**
	 * @roseuid 464368F103D5
	 */
	public CompleteForceFieldAction() {

	}

	protected void addButtonMapping(Map keyMap) {
		keyMap.put("button.link", "completeForceField");		 
	}

	public ActionForward completeForceField(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
			HttpServletResponse aResponse) throws GeneralFeedbackMessageException {	
		
		AssessmentForm assessmentForm = (AssessmentForm)aForm;
		ForceFieldAssessmentForm forceFieldAssessmentForm = (ForceFieldAssessmentForm)this.getSessionForm(aMapping, aRequest, UIConstants.CSC_ASSESS_FORCE_FIELD_ASSESSMENT_FORM, true);
...
//		clear the assessment and scsAssessment form
		assessmentForm.clearSuperviseeDetails();
		assessmentForm.clear();
		forceFieldAssessmentForm.clearAll();		
		...
		forceFieldAssessmentForm.setTaskId(taskId);
		forceFieldAssessmentForm.setAction(UIConstants.UPDATE); 
	    forceFieldAssessmentForm.setSecondaryAction("");
	    forceFieldAssessmentForm.setDefendantId(spnStr);
	    forceFieldAssessmentForm.setSupervisionBeginDate(assessmentForm.getSupervisionBeginDate());
	    forceFieldAssessmentForm.setSupervisionEndDate(assessmentForm.getSupervisionEndDate());
	    forceFieldAssessmentForm.setSupervisionPeriod(CSCAssessmentConstants.ASSESSMENT_SUPERVISION_PRD_ACTV);
	    forceFieldAssessmentForm.setAssessmentTypeId(PDCodeTableConstants.CSC_ASSESSMENTS_TYPE_ID_FORCEFIELD);
...
//		get the collection of assessmentsSummary response objects
		...
		if(assessmentCollection.size()>0){
			...
			for (int s=0; s<size; s++){
				...
				if(StringUtils.isNotEmpty(assessment.getMasterAssessmentId()) && assessmentId.equals(assessment.getMasterAssessmentId()) ) {
...
//					populate the AssessmentForm with AssessmentSummaryResponseEvent 
				    ...
				    forceFieldAssessmentForm.setSelectedAssessmentBeanId(selectedAssessBeanId.toString());
					break;
				}
			}
		}		
