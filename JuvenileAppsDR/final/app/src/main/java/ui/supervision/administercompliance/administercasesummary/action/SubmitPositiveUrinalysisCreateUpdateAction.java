//Source file: C:\\views\\CommonSupervision\\app\\src\\ui\\supervision\\administercompliance\\administercasesummary\\action\\SubmitPositiveUrinalysisCreateUpdateAction.java
...
public class SubmitPositiveUrinalysisCreateUpdateAction extends JIMSBaseAction {

	/**
	 * @roseuid 47DA9D47013F
	 */
	public SubmitPositiveUrinalysisCreateUpdateAction() {
	...
	public ActionForward finish(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
    	...
    	event.setTotalSpecimenAnalyzed(csForm.getTotalSpecimensAnalyzed());
...
//    	 TO UNIQUELY IDENTIFY MANUALLY ADDED VALUES FOR REMOVAL UPDATE PAGE AND LEGACY DATA FOR PROCESSING
		...
		for (int g = 0; g < listSize; g++){
			NonComplianceEventResponseEvent ncere = (NonComplianceEventResponseEvent) csForm.getCreate1ElementsList().get(g);
		...
		if (returnException == null) {
			csForm.setConfirmationMessage("Positive Urinalysis successfully processed.");
			// remove sort once sortResult tag is fixed
			csForm.setCreate1ElementsList(UICaseSummaryHelper.sortPositiveUrinalysisList(MessageUtil.compositeToList(response, NonComplianceEventResponseEvent.class)));
//	this should only be true on initial create of violation report	
		...
		csForm.setCurrentPositiveUrinalysisList(csForm.getCreate1ElementsList());
		csForm.setCurrentPositiveUrinalysisComments(csForm.getCreate1Comments());
		csForm.setCurrentTotalSpecimensAnalyzed(csForm.getTotalSpecimensAnalyzed());
		csForm.setCreate1ElementsList(new ArrayList());
		csForm.setCreate1Comments("");
