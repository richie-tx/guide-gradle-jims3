//Source file: C:\\views\\CommonSupervision\\app\\src\\ui\\supervision\\administercompliance\\administercasesummary\\action\\SubmitLawViolationCreateUpdateAction.java
...
public class SubmitLawViolationCreateUpdateAction extends JIMSBaseAction {

	/**
	 *@roseuid 47DA9D4602A6
	 */
	public SubmitLawViolationCreateUpdateAction() {
	...
	public ActionForward finish(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
    	...
    	UpdateNCLawViolationEvent reqEvent = null;    	
...
//    	 TO UNIQUELY IDENTIFY MANUALLY ADDED VALUES FOR REMOVAL UPDATE PAGE AND LEGACY DATA FOR PROCESSING
		...
		for (int g = 0; g < listSize; g++){
			NCLawViolationResponseEvent lvre = (NCLawViolationResponseEvent) csForm.getCreate1ElementsList().get(g);
			reqEvent = new UpdateNCLawViolationEvent();
			
			if (lvre.getLawViolationId().indexOf('M') > -1 || lvre.getLawViolationId().indexOf('L') > -1){
				reqEvent.setLawViolationId("");
			}else{
				reqEvent.setLawViolationId(lvre.getLawViolationId());
			}
	    	reqEvent.setCaseId(lvre.getCaseId());
	    	reqEvent.setCrt(lvre.getCourtId());
	    	reqEvent.setOffenseDate(lvre.getOffenseDate());
	    	reqEvent.setOffenseDegree(lvre.getOffenseDegree());
	    	reqEvent.setOffenseLevel(lvre.getOffenseLevel());
	    	reqEvent.setOffenseLiteral(lvre.getOffenseLitrel());
	    	reqEvent.setManualAdded(lvre.isManualAdded());
...
//        List list = MessageUtil.postRequestListFilter(event, NCLawViolationResponseEvent.class);
		...
		if (returnException == null) {
			csForm.setConfirmationMessage("Law Violations successfully processed.");
			// remove sort once sortResult tag is fixed
			csForm.setCreate1ElementsList(UICaseSummaryHelper.sortLawViolationList(MessageUtil.compositeToList(response, NCLawViolationResponseEvent.class)));
//	this should only be true on initial create of violation report	
		...
		csForm.setCurrentLawViolationsList(csForm.getCreate1ElementsList());
		csForm.setCurrentLawViolationsComments(csForm.getCreate1Comments());
		csForm.setCreate1ElementsList(new ArrayList());
		csForm.setCreate1Comments("");
