//Source file: C:\\views\\CommonSupervision\\app\\src\\ui\\supervision\\administercompliance\\administerviolationreport\\action\\SubmitVRPositiveUrinalysisCreateUpdateAction.java
...
public class SubmitVRPositiveUrinalysisCreateUpdateAction extends JIMSBaseAction {

	/**
	 * @roseuid 47DA9D47013F
	 */
	public SubmitVRPositiveUrinalysisCreateUpdateAction() {
	...
	public ActionForward finish(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest,
    	...
    	event.setTotalSpecimenAnalyzed(vrForm.getTotalSpecimensAnalyzed());
    	...
    	UpdateNCPositiveUAEvent reqEvent = null;    	
...
//    	 TO UNIQUELY IDENTIFY MANUALLY ADDED VALUES FOR REMOVAL UPDATE PAGE AND LEGACY DATA FOR PROCESSING
		...
		for (int g = 0; g < listSize; g++){
			NonComplianceEventResponseEvent ncere = (NonComplianceEventResponseEvent) vrForm.getCreate1ElementsList().get(g);
			reqEvent = new UpdateNCPositiveUAEvent();
			
			if (ncere.getNonComplianceEventId().indexOf('M') > -1 || ncere.getNonComplianceEventId().indexOf('L') > -1){
				reqEvent.setPositiveUAId("");
			}else{
				reqEvent.setPositiveUAId(ncere.getNonComplianceEventId());
			}
	    	reqEvent.setOccurenceDate(ncere.getDateTime());
	    	reqEvent.setSubstance(ncere.getDetails());
	    	reqEvent.setManualAdded(ncere.isManualAdded());
		...
		if (returnException == null) {
			vrForm.setConfirmationMessage("Positive Urinalysis successfully processed.");
			// remove sort once sortResult tag is fixed
			vrForm.setCreate1ElementsList(UIViolationReportHelper.sortPositiveUrinalysisList(MessageUtil.compositeToList(response, NonComplianceEventResponseEvent.class)));
//	this should only be true on initial create of violation report	
			...
			errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.generic",
							"Error occured saving Positive Urinalysis information."));
			saveErrors(aRequest, errors);
			vrForm.setCreate1ElementsList(new ArrayList());
			vrForm.setCreate1Comments("");
		}

		vrForm.setCurrentPositiveUrinalysisList(vrForm.getCreate1ElementsList());
		vrForm.setCurrentPositiveUrinalysisComments(vrForm.getCreate1Comments());
		vrForm.setCurrentTotalSpecimensAnalyzed(vrForm.getTotalSpecimensAnalyzed());
		vrForm.setCreate1ElementsList(new ArrayList());
		vrForm.setCreate1Comments("");
