 ...
 * @author rcarter
 */
public class CaseMergeQueryAction extends Action {

	private Logger log = Logger.getLogger("CaseMergeQueryAction");
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if (clrChk!=null && clrChk.equalsIgnoreCase("Y"))
		{
			...
			regform.setFromCasefile("");
			regform.setToCasefile("");
			regform.setMsg("");
		...
		//Check for null casefiles
		if (fromCasefile == null || fromCasefile.equals(" ")){
			regform.setMsg("FROM CasefileId was null.");
			return mapping.findForward("error");
		}
		if (toCasefile == null || toCasefile.equals(" ")) {
			regform.setMsg("TO CasefileId was null.");
			return mapping.findForward("error");
		}
		
		//Clear the form, reset values needed by app
		regform.clearAllResults();
		regform.setToCasefile(toCasefile);
		regform.setFromCasefile(fromCasefile);
		

		//Check for invalid casefiles
		if (casefileExists(fromCasefile)==false){
			regform.setMsg("CasefileId " + fromCasefile + " does not exist in the database.");
			return mapping.findForward("error");
		}
		if (casefileExists(toCasefile)==false){
			regform.setMsg("CasefileId " + toCasefile + " does not exist in the database.");
			return mapping.findForward("error");
		}
		
		// US 180998 changes starts
				JuvenileCasefile fromCasefileInfo = JuvenileCasefile.find(fromCasefile);
				
				String fromCasefileJuvNum = fromCasefileInfo.getJuvenileNum();
				//String fromJuv = fromCasefileInfo.getJuvenile().getJuvenileNum();
				
				JuvenileCasefile toCasefileInfo = JuvenileCasefile.find(toCasefile);
				String toCasefileJuvNum = toCasefileInfo.getJuvenileNum();
				
				if(!fromCasefileJuvNum.equalsIgnoreCase(toCasefileJuvNum)){
				    regform.setMsg("Casefiles must be for the same juvenile for Merging.");
					return mapping.findForward("error");
				}
				// US 180998 changes ENDS 
		
		//Log the query attempt
		log.info("[" + new Date() + "] ProdSupport (" + SecurityUIHelper.getLogonId()
				+ ") - Casefile Merge Query IDs: FROM " + fromCasefile + " / TO " + toCasefile);
		
		//Retrieve dependent records list;
		//regform = CaseDeleteQueryAction.retrieveCasefileRecords(regform, fromCasefile);
		regform = retrieveDataForCaseFileMerge(regform);
	 ...
	 * @return
	 */
	private ProdSupportForm retrieveDataForCaseFileMerge(ProdSupportForm  existingProdSupportForm){
	
		// Get and set CaseFile
		JuvenileCasefileResponseEvent fromCaseFileReturnEvent = retrieveCaseFile(existingProdSupportForm.getFromCasefile());
		...
		ArrayList caseFileActivitiesList = (ArrayList)ActivitiesHelper.getActivities(existingProdSupportForm.getFromCasefile(), null, null, null, null);
		...
		getAssignments.setCasefileId(existingProdSupportForm.getFromCasefile());
		...
		getAssignmentHistoryEvent.setCasefileId(existingProdSupportForm.getFromCasefile());
		...
		CasefileClosingResponseEvent respEvent = UIJuvenileCasefileClosingHelper.
			getCasefileClosingDetails(existingProdSupportForm.getFromCasefile());
		...
		getInterviewsEvent.setCasefileId(existingProdSupportForm.getFromCasefile());
		...
		getInterviewDocs.setCasefileId(existingProdSupportForm.getFromCasefile());
		...
		riskAssessEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getJuvenileProgramRerralsEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		benefitAssessmentsEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		juvenileCasefileSupervisionRulesEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getCalendarEventContextEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getEventTasksEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getNTTasksEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getCaseplansEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getJuvProgramAssignmentHistoriesEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		getTraitsEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		//Get and set booking supervision number and current supervision Number (US 52411 Task 155978 STARTS
		
		GetJJSDetentionBySupervisionNumEvent getRecordsBySupervision = (GetJJSDetentionBySupervisionNumEvent)EventFactory.getInstance(JuvenileCaseControllerServiceNames.GETJJSDETENTIONBYSUPERVISIONNUM);
		getRecordsBySupervision.setBookingSupervisionNum(existingProdSupportForm.getFromCasefile());
		getRecordsBySupervision.setCurrentSupervisionNum(existingProdSupportForm.getFromCasefile());
		CompositeResponse getDetentionbySupNumResponse = MessageUtil.postRequest(getRecordsBySupervision);
		ArrayList getDetentionRecordsList = (ArrayList) MessageUtil.compositeToCollection(getDetentionbySupNumResponse, JuvenileDetentionFacilitiesResponseEvent.class);
		if(getDetentionRecordsList != null && getDetentionRecordsList.size() >0){
		    existingProdSupportForm.setFacilityDetentionCount(getDetentionRecordsList.size());
		    existingProdSupportForm.setFacilityDetentionList(getDetentionRecordsList);
		}
		GetJJSHeaderBySupervisionNumEvent getJJSHeaderRecords = (GetJJSHeaderBySupervisionNumEvent)EventFactory.getInstance(JuvenileCaseControllerServiceNames.GETJJSHEADERBYSUPERVISIONNUM);
		getJJSHeaderRecords.setBookingSupervisionNum(existingProdSupportForm.getFromCasefile());
		dispatch.postEvent(getJJSHeaderRecords);
		IEvent replyEvent = dispatch.getReply();
		CompositeResponse response = (CompositeResponse)replyEvent;
		JuvenileFacilityHeaderResponseEvent juvFacHeaderRespEvent =(JuvenileFacilityHeaderResponseEvent)
			MessageUtil.filterComposite(response,JuvenileFacilityHeaderResponseEvent.class);
		if(juvFacHeaderRespEvent != null){
		    existingProdSupportForm.setFacilityDetentionCount(getDetentionRecordsList.size() + 1); //there will only be one entry in JJS header table
		    existingProdSupportForm.setHeaderInfo(juvFacHeaderRespEvent);
		}
		...
		GetDrugTestingInfoEvent getDrugTestingInfoEvent = (GetDrugTestingInfoEvent)
	        ...
	        getDrugTestingInfoEvent.setCasefileId(existingProdSupportForm.getFromCasefile());
	        CompositeResponse replyEvent1 = MessageUtil.postRequest(getDrugTestingInfoEvent);
    	        Collection<DrugTestingResponseEvent> drugTestingInfos = MessageUtil.compositeToCollection(replyEvent1, DrugTestingResponseEvent.class);
        	if ( drugTestingInfos != null
        	 ...
        	 else
        	 {
        	  ...
        	  getSubstanceAbuseInfoEvent.setCasefileId( existingProdSupportForm.getFromCasefile() );
		...
		//Get and set booking supervision number and current supervision Number (US 52411 Task 155978 Ends
