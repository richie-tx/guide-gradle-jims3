...
public class SubmitResidentialAssessmentAction extends LookupDispatchAction
{
     ...
     */
    public SubmitResidentialAssessmentAction()
    {
            ...
            HttpServletResponse aResponse)
    {
        ...
        event.setAssessmentType(RiskAnalysisConstants.RISK_TYPE_RESIDENTIAL);
        ...
        if (resp != null && resp.isMessage() == true) {
        	...
        	errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.assessmenttypecannotbedonewithinhour", 
        		RiskAnalysisConstants.RISK_TYPE_RESIDENTIAL, riskForm.getCasefileID()));
	        ...
	        saveEvt.setAssessmentType(RiskAnalysisConstants.RISK_TYPE_RESIDENTIAL);
	        saveEvt.setRiskFormulaId(riskForm.getRiskFormulaId());
	
	        Iterator ite = riskForm.getProcessedQuestionAnswers().iterator();
	        while (ite.hasNext())
	        {
	            RiskQuestionAnswerEvent qa = (RiskQuestionAnswerEvent) ite.next();
	            saveEvt.addRequest(qa);
	        }       
	 ...
	 */
	public ActionForward updateResidentialRiskAnalysisOverrideStatus(ActionMapping aMapping, ActionForm aForm, 
