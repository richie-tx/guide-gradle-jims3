...
public class PerformJuvenilePurgeAction extends Action
{
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		log.info("BEGIN JUVENILE PURGE - " + " LogonId: " + SecurityUIHelper.getLogonId());
		...
		log.info(" PURGED USING JUVENILE NUM: " + juvenileId + "Logon Id: " + 
		  ...
		  //court records purging
			...
			if (crtdktRespEvts != null && !crtdktRespEvts.isEmpty())
			{
				...
				if (crtDocResp != null)
				{
				    ...
				    updateCourtSeal.setAction("purge");
					...
					//ActionErrors errors = new ActionErrors();
					errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.generic", "Court Records failed to Purge"));
					saveErrors(request, errors);
					return mapping.findForward("error");
				    }
			...
			if (docketResponses != null && !docketResponses.isEmpty())
			{
				...
				if (crtDetDocResp != null)
				{
				    ...
				    updateDetCourtSeal.setAction("purge");
					...
					if (errRes != null){
						...
						//ActionErrors errors = new ActionErrors();
						errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.generic", "Detention Records failed to Purge."));
						saveErrors(request, errors);
						return mapping.findForward("error");
					}					
			...
			if (petResponses != null && !petResponses.isEmpty())
			{
				...
				if (petResp != null)
				{
					...
					if (errRes != null){
						...
						//ActionErrors errors = new ActionErrors();
						errors.add(ActionErrors.GLOBAL_MESSAGE, new ActionMessage("error.generic", "Petition Records failed to Purge."));
						saveErrors(request, errors);
