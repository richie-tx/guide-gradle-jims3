 ...
 * @author rcarter
 * 
 * Query to retrieve Facility Detention records to delete
 */

public class DeleteFacilityDetentionQueryAction extends Action {

	private Logger log = Logger.getLogger("DeleteFacilityDetentionQueryAction");
	...
	if (clrChk != null && clrChk.equalsIgnoreCase("Y")) {
		...
		regform.setMsg("");
		regform.setIsLastSeqNum("false");
		regform.setLastSeqNum("");
		regform.setNewLastSeqNum("");
	...
	String sequenceNum = regform.getSequenceNumber();
	...
	if (editFlag != null && editFlag.equalsIgnoreCase("Y")) {
		...
		if (detentionId != null && detentionId.equals("") == false){
			...
			if(singleDetentionRecordList != null && singleDetentionRecordList.size() > 0){
				...
				regform.setSequenceNum(resp.getSequenceNum());
				...
				regform.setMsg("There were no records found for the following:" +  "\nJuvenileId: " + 
	...
	if(detentionId != null && detentionId.length() > 0){
		...
		String secondLastSeqNum = getSecondLastSeqNum(juvenileId);
		
		if (secondLastSeqNum.length() > 0 ) {
		    regform.setNewLastSeqNum(secondLastSeqNum);
		}
		if(detentionRecordsList != null && detentionRecordsList.size() > 1){
			regform.setFacilityDetentionList(detentionRecordsList);
			regform.setLastSeqNum( detentionRecordsList.get(0).getSequenceNum() );
...
private ArrayList retrieveDetentionRecords(String juvenileId, String referralNum, String sequenceNum, String logonId){
	...
	System.out.println("Detention sequence number: " + sequenceNum );
...
private String getSecondLastSeqNum(String juvenileNum) {
    String seqNum = "";
    ArrayList<ProductionSupportFacilityDetentionResponseEvent> facilityDetentionList =  new ArrayList<ProductionSupportFacilityDetentionResponseEvent>();
    facilityDetentionList = retrieveDetentionRecords(juvenileNum, "", "", "");
    
    if ( facilityDetentionList != null
	    && facilityDetentionList.size() > 1 ) {
	seqNum =  facilityDetentionList.get(1).getSequenceNum();
    }
