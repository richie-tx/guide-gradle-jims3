 ...
 *
 */
public class HandleProcessReferralsOffenseSearchAction extends JIMSBaseAction
{
	 /* (non-Javadoc)
    ...
    */
   public HandleProcessReferralsOffenseSearchAction() 
   {
   ...
   public ActionForward findOffenses(ActionMapping aMapping, ActionForm aForm, HttpServletRequest aRequest, HttpServletResponse aResponse) 
	...
	else
	{
		...
		if (ocEvent.getAlphaCode().indexOf(alphaCode) == 0)
		{
		    temp1.add(ocEvent);
		}
	    }
	...
	else
	{
		...
		if (ocEvent.getShortDescription().indexOf(shortDesc) > -1 )
		{
		    temp2.add(ocEvent);
		}
	    }
	...
	else
	{
		...
		if ((ocEvent.getCategory().indexOf(category) == 0))//BUG 84401 (Bug doesn't say this issue)
		//if ((ocEvent.getCategory().indexOf(category) == 0) && ((ocEvent.getDpsCode().length() > 1))){ //commented for BUG 84401
		{
		    temp4.add(ocEvent);
		}
	    }
	...
	if (temp4.size() == 0)
	{
	    form.setMessage("No records found for supplied search criteria");
	}
	temp3 = null;
	temp4 = null;
	category = null;

	return aMapping.findForward("offenseSearchSuccess");
    }
    ...
    */
	...
	for (int x = 0; x < wrkList.size(); x++)
	{
	    ...
	    {
		form.setOffenseCode(jcoEvent.getAlphaCode());
		form.setOffenseCodeDesc(jcoEvent.getShortDescription());
	...
	if(form.getAction()!= null && form.getAction().equalsIgnoreCase("updateReferral"))
	    forwardStr = "updateRefAddOffense";
	else if(form.getAction()!= null && form.getAction().equalsIgnoreCase("newJuvCreateRef"))
	    forwardStr = "newJuvCreateRefAddOffense";
	else
	    forwardStr = "successAddOffense";
    ...
    */
	...
	if(form.getAction()!= null && form.getAction().equalsIgnoreCase("updateReferral"))
	    forwardStr = "updateRefAddOffense";
	if(form.getAction()!= null && form.getAction().equalsIgnoreCase("newJuvCreateRef"))
	    forwardStr = "newJuvCreateRefAddOffense";
	return aMapping.findForward(forwardStr);
    }
    ...
    */
	...
	if(form.getAction()!=null && !form.getAction().equals("") && form.getAction().equalsIgnoreCase("updateReferral"))
	    forwardStr = "updateRefAddOffense";
	if(form.getAction()!= null && form.getAction().equalsIgnoreCase("newJuvCreateRef"))
	    forwardStr = "newJuvCreateRefAddOffense";
	return aMapping.findForward(forwardStr);
