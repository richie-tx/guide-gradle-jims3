...
/**
 * This class is used only to retrieve and manipulate simple code tables and the codeResponseEvents these are the basic methods that anyone should 
 * need when working with code tables, additional methods should only be needed after careful consideration as to whether there is a more appropriate way to
 * do the job.
 */
public class SimpleCodeTableHelper extends CodeTableHelper{
		...
		public static List<CodeResponseEvent> getActiveCodes(List<CodeResponseEvent> raceCodes)
		{
			     ...
			     if(mySelectedCodeRespEvnts!=null && mySelectedCodeRespEvnts.size()>0){
			     	 for (int i = 0; i < mySelectedCodeRespEvnts.size(); i++)
			         {
			     	 	...
			     	 	if(myObj!=null && (myObj instanceof CodeResponseEvent)){
				         	...
				         	if(findVal!=null && myRetVal.getCodeId()!=null && myRetVal!=null && myRetVal.getDescription()!=null && myRetVal.getCode().equals(findVal)){
